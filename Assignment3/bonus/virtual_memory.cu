...
__device__ void init_invert_page_table(VirtualMemory *vm) {
  ...
  for (int i = 0; i < vm->PAGE_ENTRIES; i++) {
    ...
    vm->invert_page_table[i + 3 * vm->PAGE_ENTRIES] = vm->PID;    //Process ID     4KB
  }
}
...
__device__ u32 paging(VirtualMemory *vm, u32 pageNum, u32 offset, u32 is_read) {
  ...
  for (int i = 0; i < vm->PAGE_ENTRIES; i++) {
    //Everytime modify or search the invert_page_table, check the PID first.
    if (vm->invert_page_table[i + 3 * vm->PAGE_ENTRIES] == vm->PID) {
      if (vm->invert_page_table[i] == 0) {
  ...
  *(vm->pagefault_num_ptr) = *(vm->pagefault_num_ptr) + 1;
  ...
  for (int i = 0; i < vm->PAGESIZE; i++) {
    if (vm->invert_page_table[leastUsedPage + 3 * vm->PAGE_ENTRIES] == vm->PID) {
